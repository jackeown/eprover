/*-----------------------------------------------------------------------
  ...
  A script to evaluate clauses under E's clause evaluation functions.
  It's adopted from "eprover.c" to get some initial boilerplate processing before
  evaluation.
...
//    }
   ...
   if(!success)
   {
    ...
    printf("# Unprocessed Size: %ld\n", proofstate->unprocessed->members);
    assert(proofstate->processed_neg_units->members == 0);
    assert(proofstate->processed_non_units->members == 0);
    assert(proofstate->processed_pos_rules->members == 0);
    assert(proofstate->processed_pos_eqns->members  == 0);

    PDArray_p wfcb_list = proofcontrol->hcb->wfcb_list;
    long wfcb_no = proofcontrol->hcb->wfcb_no;
    PStack_p wfcb_stack = proofcontrol->wfcbs->wfcb_set;
    PStack_p wfcb_names = proofcontrol->wfcbs->names;
    printf("# proofcontrol->hcb->wfcb_no: %ld\n", wfcb_no);
    printf("# proofcontrol->hcb->wfcb_list->size: %ld\n", wfcb_list->size);



    // Print out headers
    for(int i=0; i<wfcb_no; i++){
        WFCB_p wfcb = (WFCB_p) (wfcb_list->array[i]).p_val;
        char* name = "NoName";
        for(int j=0; j<wfcb_names->size; j++){
              if ((WFCB_p) (wfcb_stack->stack[j]).p_val == wfcb){
                  name = wfcb_names->stack[j].p_val;
              }
        }
        printf("%s, ", name);
    }
    ...
    // Print out clause evaluations
    for(Clause_p handle = proofstate->unprocessed->anchor->succ;
        handle != proofstate->unprocessed->anchor;
        handle = handle->succ){
      ...
      //   printf(", ");
        for(int i=0; i<wfcb_no; i++){
            WFCB_p wfcb = (WFCB_p) (wfcb_list->array[i]).p_val;
            // WFCB_p wfcb = (WFCB_p) (wfcb_stack->stack[i]).p_val;

            double evaluation = wfcb->wfcb_eval(wfcb->data, handle);
            printf("%lf, ", evaluation);
        }
        printf("\n");
    }
